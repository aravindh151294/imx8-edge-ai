...
try:
    ...
    print(f"NPU not available ({e}), falling back to CPU 4-thread")
    interpreter = tflite.Interpreter(
        model_path=MODEL_PATH,
        num_threads=4
    )
...
print(f"\n=== iMX8MP Inference Benchmark ===")
print(f"Accelerator:      {'NPU (VX Delegate)' if npu_available else 'CPU 4-thread'}")
